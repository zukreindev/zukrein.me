<script>
	...
	onMount(async () => {
		fetch('https://api.lanyard.rest/v1/users/888795101843820564').then((e) => {
			e.json().then((e) => {
				dicordid = e.data.discord_user.id;
				discordpp = e.data.discord_user.avatar;
				discordstatus = e.data.discord_status;
				discordpp = discordpp
